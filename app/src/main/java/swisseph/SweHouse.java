...
//Never used anywhere//  }
  ...
  */
  ...
  int swe_houses(double tjd_ut,
    ...
    Log.e("SwissEph","------- swe_houses --------");
    Log.e("SwissEph","tjde " + tjde);
    ...
    Log.e("SwissEph","hsys " + hsys + " ito " + ito);
    ...
    Log.e("SwissEph","The ecliptic obliquity eps_mean = sl.swi_epsiln(tjde, 0) * SwissData.RADTODEG ");
    Log.e("SwissEph","eps_mean " + eps_mean);
    sl.swi_nutation(tjde, 0, nutlo);
    for (i = 0; i < 2; i++)
      nutlo[i] *= SwissData.RADTODEG;
    Log.e("SwissEph","nutlo " + Arrays.toString(nutlo));
    ...
    Log.e("SwissEph","armc " + armc);
    Log.e("SwissEph","iflag " + iflag);
    if ((iflag & SweConst.SEFLG_SIDEREAL)!=0) {
      ...
      Log.e("SwissEph","retc " + retc);
      Log.e("SwissEph","ecliptic obliquity eps_mean + nutlo[1] " + (eps_mean + nutlo[1]));
    }
  ...
  */
  int swe_houses_armc(double armc,
                      ...
                      double ascmc[],
                      int aOffs) {
    Log.e("SwissEph","----- swe_houses_armc -----");
    Houses h=new Houses();
    int i, retc = 0;
    int ito;
    if (Character.toUpperCase((char)hsys) == 'G') {
    ...
    return retc;
           ...
           * @param ph geographic latitude
           * @param e  ecliptic obliquity
           * @param az armc
           * @return
           */
  ...
  String swe_house_name(int hsys) {
    switch (Character.toUpperCase((char)hsys)) {
      ...
      default: return "Placidus";
           ...
           *
           /* *********************************************************
           * @param th Arguments: th = sidereal time (angle 0..360 degrees
           * @param hsy hsy = letter code for house system;
           *                   A  equal
           *                   E  equal
           *                   B  Alcabitius
           *                   C  Campanus
           ...
           *                   Y  APC houses
           * @param fi fi = geographic latitude
           * @param ekl ekl = obliquity of the ecliptic
           * @param iteration_count iteration_count = number of iterations in
  ...
  private int CalcH(double th, double fi, double ekl, char hsy,
    ...
    double sine, cose;
    double[] x = new double[3]; /* BK 14.02.2006 */
    double krHorizonLon;
    cose  = cosd(ekl);
    sine  = sind(ekl);
    tane  = tand(ekl);
