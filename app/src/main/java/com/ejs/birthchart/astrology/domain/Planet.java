package com.ejs.birthchart.astrology.domain;

import com.ejs.birthchart.astrology.builder.PlanetBuilder;
import com.ejs.birthchart.astrology.json.Convertor;
 ...
 * This class should not be used alone. Use {@link PlanetBuilder}
 * to create the correct instance of this class.
 * 
...
public class Planet extends Ephemeris{
	 ...
	 * @param planets List of planets for position calculation. Constants of planets are in {@link SweConst}.
	...
	public Planet( LocalDateTime event, List<Integer> planets, Coordinates coords, int iflag ) {
		...
		this.planets = planets;
		this.coords = coords;
		this.iflag = SweConst.SEFLG_MOSEPH | SweConst.SEFLG_SPEED | SweConst.SEFLG_TOPOCTR;
	...
	private Map<String, List<Double>> calculatePlanets( List<Integer> planets, SwissEph calculator, SweDate date, int flags ) {
		...
		for (Integer planet : planets) {
			...
			int ret = calculator.swe_calc_ut(date.getJulDay(),
					planet,
					flags,
					xp,
					serr);
			...
			values.add(xp[1]); //latitude
			values.add(xp[3]); //speed in longitude
			
			data.put( getPlanetName(planet, calculator), values);
		}
	 ...
	 */
	private String getPlanetName(int planet, SwissEph calculator) {
				
		String name = calculator.swe_get_planet_name(planet);
	...
	public String toString() {
		...
		sb.append("[ UTC: " + this.sd.getDate(0));
