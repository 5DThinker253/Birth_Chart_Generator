 ...
 * This class should not be used alone. Use {@link com.ejs.birthchart.astrology.builder.CuspBuilder}
...
public class Cusp extends Ephemeris{
	...
	public Cusp( LocalDateTime event, Coordinates coords, Integer houseSystem, int iflag) {
		...
		sw = new SwissEph();
		sd = new SweDate();
		sd.setDate(event.getYear(), event.getMonthValue(), event.getDayOfMonth(), event.getHour() + event.getMinute()/60.0 + event.getSecond()/3600.0);
		
		if( (this.iflag & 0xF0000) ==  SweConst.SEFLG_SIDEREAL ) {
			sw.swe_set_sid_mode( this.iflag & 0x00FF );
		}
		sw.swe_set_sid_mode( SweConst.SE_SIDM_FAGAN_BRADLEY);
		this.iflag = SweConst.SEFLG_MOSEPH;
