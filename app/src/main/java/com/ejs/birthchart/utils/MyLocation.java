...
public class MyLocation {
    ...
    public boolean getLocation(Context context, LocationResult result) {
        ...
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED &&
            ...
            if (gps_enabled)
                lm.requestLocationUpdates(LocationManager.GPS_PROVIDER, 2000, 500, locationListenerGps);
            if (network_enabled)
                lm.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 2000, 500, locationListenerNetwork);
        }
        timer1 = new Timer();
